...
## `wrap-roadmap-groups.py`

Wraps leaf `TextInput` controls in the VLSI roadmap JSON files
(`src/data/roadmaps/{roadmap}/{roadmap}.json`) in `__group__` controls so the
renderer makes them clickable, and writes `topic-manifest.json` (roadmap ->
list of `{slug, text, category}`) listing the matching
`content/{slug}@{slug}.md` files that need to be written.

```bash
python3 scripts/wrap-roadmap-groups.py [roadmap-id ...]
```

Run with no arguments to (re-)process all 7 VLSI roadmaps. Use this after
adding or editing a roadmap's JSON in the Balsamiq editor and before writing
its content markdown files.
